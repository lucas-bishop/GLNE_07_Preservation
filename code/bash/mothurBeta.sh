...
echo PROGRESS: Calculating "${SHARED}" beta diversity and subsampling to "${READCOUNT}" reads.

# Calculating alpha and beta diversity
# If a sample doesn't have enough reads, it'll be eliminated from the analysis
mothur "#dist.shared(shared="${SHARED}", calc=sharedsobs-thetayc-braycurtis, subsample="${READCOUNT}")"
