...
# Read in data
data <- read_tsv("data/process/shared_file.tsv")
...
data <- data %>%
  mutate(dx = case_when(
    diagnosis == "Adenoma" ~ "normal",
    diagnosis == "Normal" ~ "normal",
    diagnosis == "AdvAdenoma" ~ "cancer",
    diagnosis == "Carcinoma" ~ "cancer"
  )) %>%
  select(-Group, -diagnosis, -condition, -sample_ID) %>%
  drop_na()
